...
namespace DPSBase
{
    ...
    public static class StreamWriteWithTimeout
    {
        ...
        /// Allows a minimum of 50 milliseconds for any write.
        /// </summary>
        ...
        /// <param name="timeoutPerKBWriteMS">The maximum time to allow for write to complete per KB</param>
        /// <returns>The average time in milliseconds per KB written</returns>
        public static double Write(byte[] sendBuffer, int bufferStart, int bufferLength, Stream destinationStream, int writeBufferSize, double timeoutPerKBWriteMS)
        {
            int totalBytesCompleted = 0;
            bool innerExceptionOccured = false;
            AutoResetEvent writeCompletedEvent = new AutoResetEvent(false);

            int writeWaitTimeMS = Math.Max(50,(int)((writeBufferSize / 1024.0) * timeoutPerKBWriteMS));
            ...
            do
            {
                ...
                if (!writeCompletedEvent.WaitOne(writeWaitTimeMS) || innerExceptionOccured)
        ...
        public static double Write(Stream inputStream, int inputStart, int inputLength, Stream destinationStream, int writeBufferSize, double timeoutPerKBWriteMS)
        {
            ...
            int writeWaitTimeMS = Math.Max(50,(int)((sendBuffer.Length / 1024.0) * timeoutPerKBWriteMS));
