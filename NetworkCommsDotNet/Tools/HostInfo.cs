...
namespace NetworkCommsDotNet
{
    ...
    public static class HostInfo
    {
        ...
        /// If set <see cref="FilteredLocalIPAddresses()"/> will only return addresses corresponding with specified adaptors names. 
        ...
        /// <summary>
        /// Restricts the IPAdddresses that are returned by <see cref="FilteredLocalIPAddresses()"/>.
        ...
        /// The order of provided ranges determines the order of IPAddresses returned by <see cref="FilteredLocalIPAddresses()"/>.
        /// </summary>
        public static IPRange[] RestrictLocalIPAddressRanges { get; set; }
        ...
        /// If <see cref="RestrictLocalIPAddressRanges"/> has been set only returns matching addresses ordered in descending 
        /// preference. i.e. Most preferred at [0].
        /// </summary>
        /// <returns></returns>
        public static List<IPAddress> FilteredLocalIPAddresses()
        {
            ...
            foreach (var hName in Windows.Networking.Connectivity.NetworkInformation.GetHostNames())
            {
                ...
                {
                     if (RestrictLocalIPAddressRanges != null)
                    {
                        bool valid = false;

                        for (int i = 0; i < RestrictLocalIPAddressRanges.Length; i++)
                            valid |= RestrictLocalIPAddressRanges[i].Contains(hName.DisplayName);
                ...
                foreach (var address in unicastAddresses)
                {
                    if (address.Address.AddressFamily == AddressFamily.InterNetwork || address.Address.AddressFamily == AddressFamily.InterNetworkV6)
                    {
                        if (!IPTools.IsAddressInSubnet(address.Address, autoAssignSubnetv4, autoAssignSubnetMaskv4))
                        {
                            ...
                            if (RestrictLocalIPAddressRanges != null)
                            {
                                if (IPRange.Contains(RestrictLocalIPAddressRanges, address.Address))
                                    allowed = true;
                            }
                            else
                                allowed = true;
            ...
            if (RestrictLocalIPAddressRanges != null)
            {
                validIPAddresses.Sort((a, b) =>
                {
                    for (int i = 0; i < RestrictLocalIPAddressRanges.Length; i++)
                    {
                        if (RestrictLocalIPAddressRanges[i].Contains(a))
                        {
                            if (RestrictLocalIPAddressRanges[i].Contains(b))
                                return 0;
                            else
                                return -1;
                        }
                        else if (RestrictLocalIPAddressRanges[i].Contains(b))
                            return 1;
                    }
        ...
        internal static void ShutdownHostTools(int threadShutdownTimeoutMS = 1000)
        {
#if !WINDOWS_PHONE && !ANDROID && !NETFX_CORE
            try
            {
